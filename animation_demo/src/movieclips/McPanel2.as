package movieclips
{
	...
	public class McPanel2 extends Bitmap
	{
		private var _coins:Vector.<AnimationModel>;
		private var _unsorted:Vector.<AnimationModel>;
		...
		public function McPanel2()
		{
			_coins = new Vector.<AnimationModel>();
			_unsorted = new Vector.<AnimationModel>();
		...
		{
			addCoins();
			_text = new AnimationModel(new McModel("big_win", Config.BIG_WIN_FRAMES_NUM, new Point(), 1), false, 1, -1, 18);
		...
		private function addCoins():void
		{
			for (var i:int = 0; i < 500; i++)
			{
				...
				var animModel:AnimationModel = new AnimationModel(coin, true, 1, 0, 18);

				_coins.push(animModel);
				_unsorted.push(animModel);
			}
		...
		private function tryToAddCoin():void
		{
			...
			var animModel:AnimationModel = _unsorted[_currentStart];

			var coin:McModel = animModel.mc as McModel;
			...
			AnimationManager.startAnimation(animModel, "coin_" + _currentStart.toString());
